...
export const FormularioLocacao = () => {
  ...
  return <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-4xl mx-auto">
        <Card className="shadow-lg">
          <CardHeader className="bg-primary text-primary-foreground rounded-t-lg">
            <CardTitle className="text-3xl font-bold text-center flex items-center justify-center gap-3 text-slate-100">
          ...
          <CardContent className="p-8 space-y-8">
            <form onSubmit={handleSubmit} className="space-y-8">
              <CamposBasicos apartamento={formData.apartamento} hospede={formData.hospede} telefone={formData.telefone} apartamentosDisponiveis={apartamentosDisponiveis} onApartamentoChange={value => setFormData({
              ...formData,
              apartamento: value
            })} onHospedeChange={value => setFormData({
              ...formData,
              hospede: value
            })} onTelefoneChange={value => setFormData({
              ...formData,
              telefone: value
            })} />
              ...
              <div className="bg-gray-50 rounded-xl p-6 border">
                ...
                <CamposDatas dataEntrada={formData.dataEntrada} dataSaida={formData.dataSaida} apartamento={formData.apartamento} onDataEntradaChange={value => setFormData({
                ...formData,
                dataEntrada: value
              })} onDataSaidaChange={value => setFormData({
                ...formData,
                dataSaida: value
              })} />
              </div>
              ...
              <div className="bg-gray-50 rounded-xl p-6 border">
                ...
                <CamposValores valorLocacao={formData.valorLocacao} primeiroPagamento={formData.primeiroPagamento} primeiroPagamentoPago={formData.primeiroPagamentoPago} primeiroPagamentoForma={formData.primeiroPagamentoForma} segundoPagamento={formData.segundoPagamento} segundoPagamentoPago={formData.segundoPagamentoPago} segundoPagamentoForma={formData.segundoPagamentoForma} taxaLimpeza={formData.taxaLimpeza} onValorLocacaoChange={value => handleCurrencyChange('valorLocacao', value)} onPrimeiroPagamentoChange={value => handleCurrencyChange('primeiroPagamento', value)} onPrimeiroPagamentoPagoChange={checked => setFormData({
                ...formData,
                primeiroPagamentoPago: checked
              })} onPrimeiroPagamentoFormaChange={value => setFormData({
                ...formData,
                primeiroPagamentoForma: value
              })} onSegundoPagamentoPagoChange={checked => setFormData({
                ...formData,
                segundoPagamentoPago: checked
              })} onSegundoPagamentoFormaChange={value => setFormData({
                ...formData,
                segundoPagamentoForma: value
              })} />
              </div>

              <div className="bg-gray-50 rounded-xl p-6 border">
                <h3 className="text-lg font-semibold text-gray-800 mb-4">Resumo Financeiro</h3>
                <CamposCalculados valorLocacao={formData.valorLocacao} taxaLimpeza={formData.taxaLimpeza} comissao={comissao} valorProprietario={valorProprietario} />
              </div>
              ...
              })} placeholder="Observações adicionais..." rows={3} className="mt-2" />
              </div>

              <div className="bg-gray-50 rounded-xl p-6 border">
                <h3 className="text-lg font-semibold text-gray-800 mb-4">Pagamento do Proprietário</h3>
                <CamposPagamento proprietarioPago={formData.proprietarioPago} dataPagamentoProprietario={formData.dataPagamentoProprietario} onProprietarioPagoChange={checked => setFormData({
                ...formData,
                proprietarioPago: checked
              })} onDataPagamentoProprietarioChange={value => setFormData({
                ...formData,
                dataPagamentoProprietario: value
              })} />
              </div>

              <Button type="submit" className="w-full font-semibold py-4 text-lg bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg transform transition-all duration-200 hover:scale-[1.02]">
                Cadastrar Locação
              </Button>
            </form>
          </CardContent>
      ...
      <ConfirmacaoNovoRegistro open={showConfirmacao} onOpenChange={setShowConfirmacao} titulo="Locação cadastrada com sucesso!" descricao="A locação foi salva no sistema e está disponível na lista de locações." onNovoRegistro={handleNovoRegistro} onIrParaLista={handleIrParaLista} showWhatsAppButton onEnviarWhatsApp={handleEnviarWhatsAppConfirmacao} showLinkPagamentoButton onGerarLinkPagamento={handleGerarLinkPagamento} />

      <WhatsAppModalLocacao open={showWhatsApp} onOpenChange={setShowWhatsApp} locacao={locacaoCadastrada} apartamentos={apartamentos} modelos={modelos} onProcessarTemplate={processarTemplate} onEnviar={handleEnviarWhatsApp} enviando={enviandoWhatsApp} />

      <LinkPagamentoModal open={showLinkPagamento} onOpenChange={setShowLinkPagamento} locacao={locacaoCadastrada} />
    </div>;
};